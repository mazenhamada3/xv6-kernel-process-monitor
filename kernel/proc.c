...
//p3
// at 470 int sched_mode = SCHED_ROUND_ROBIN; // Default policy
struct spinlock metrics_lock;       // Lock for metrics
uint64 total_turnaround_time = 0;   // Accumulated turnaround time
uint64 total_waiting_time_sum = 0;  // Accumulated waiting time
uint64 finished_processes = 0;      // Number of completed processes
...
scheduler(void)
{
  ...
  for(;;){
    ...
    // ----------------------------------------------------
      ...
      // Iterate ALL processes to find the one with the smallest creation_time
      for(p = proc; p < &proc[NPROC]; p++){
        acquire(&p->lock);
        if(p->state == RUNNABLE){
          // If we haven't found a candidate yet, OR this one is older (smaller time)
      ...
      if(min_p != 0){
        p = min_p; // min_p is already locked
    ...
    // ----------------------------------------------------
      ...
      // Iterate ALL processes to find the one with lowest priority value (Highest Importance)
      for(p = proc; p < &proc[NPROC]; p++){
        acquire(&p->lock);
        if(p->state == RUNNABLE){
          // If we haven't found a candidate yet, OR this one has higher priority (smaller value)
          //"tie breaker" p3
      ...
      // If we found a process to run
      if(high_p != 0){
        p = high_p; // high_p is already locked
...
yield(void)
{
  ...
  // --- 1. FCFS Protection ---
  // If in FCFS mode, ignore the timer and keep running.
